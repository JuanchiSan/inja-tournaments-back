...
public class CityAdaptor : DataAdaptor
{
  ...
  {
    IEnumerable<City> DataSource = _db.Cities.ToList();
    ...
    int count = DataSource.Cast<City>().Count();
    if (dm.Skip != 0)
    {
      //Paging
  ...
  {
    var ModelValue = (value as City);
    var data = _db.Cities.Where(x => x.Id == ModelValue.Id).SingleOrDefault();
  ...
  {
    City ord = _db.Cities.Find((int)value);
